...
FPS = 50
# affects how fast-paced the game is, forces should be adjusted as well (Don't touch)
SCALE = 60.0
...
RACKETPOLY = [(-10, 20), (+5, 20), (+5, -20), (-10, -20),
              (-18, -10), (-21, 0), (-18, 10)]
RACKETFACTOR = 1.2
...
class HockeyEnv(gym.Env, EzPickle):
    ...
    def __init__(self, keep_mode: bool = True, mode: int | str | Mode = Mode.NORMAL, verbose: bool = False):
        ...
        self.observation_space = spaces.Box(-np.inf,
                                            np.inf, shape=(18,), dtype=np.float32)
    ...
    def _create_world(self):
        ...
        poly = [(-10, (H - 1) / 2 * SCALE - GOAL_SIZE),
                (10, (H - 1) / 2 * SCALE - GOAL_SIZE - 7), (10, -5), (-10, -5)]
        self.world_objects.append(_create_wall(
            (W / 2 - 245 / SCALE, H - .5), [(x, -y) for x, y in poly]))
        self.world_objects.append(_create_wall(
            (W / 2 - 245 / SCALE, .5), poly))

        self.world_objects.append(
            _create_wall((W / 2 + 245 / SCALE, H - .5), [(-x, -y) for x, y in poly]))
        self.world_objects.append(_create_wall(
            (W / 2 + 245 / SCALE, 0.5), [(-x, y) for x, y in poly]))
    ...
    def reset(self, one_starting=None, mode=None, seed=None, options=None):
        self._destroy()
        self.seed(seed)
        self.world.contactListener_keepref = ContactDetector(
        ...
        poly = [(-10, GOAL_SIZE), (10, GOAL_SIZE),
                (10, -GOAL_SIZE), (-10, -GOAL_SIZE)]
        ...
        if (is_player_one and player.position[0] > CENTER_X - ZONE) \
            ...
            if is_player_one:
                if player.linearVelocity[0] > 0:
                    force[0] = -2 * player.linearVelocity[0] * \
                        player.mass / self.timeStep
                force[0] += -1 * (player.position[0] - CENTER_X) * \
                    player.linearVelocity[0] * player.mass / self.timeStep
            else:
                if player.linearVelocity[0] < 0:
                    force[0] = -2 * player.linearVelocity[0] * \
                        player.mass / self.timeStep
            ...
            player.ApplyForceToCenter(self._check_boundaries(
                force, player, is_player_one).tolist(), True)
            return

        if speed < max_speed:
            player.linearDamping = 5.0
            player.ApplyForceToCenter(self._check_boundaries(
                force.tolist(), player, is_player_one), True)
        else:
            player.linearDamping = 20.0
    ...
    def _get_info(self):
        ...
        if self.puck.position[0] < CENTER_X and self.puck.linearVelocity[0] <= 0:
            dist_to_puck = dist_positions(
                self.player1.position, self.puck.position)
            max_dist = 250. / SCALE
    ...
    def get_info_agent_two(self):
        ...
        if self.puck.position[0] > CENTER_X and self.puck.linearVelocity[0] >= 0:
            dist_to_puck = dist_positions(
                self.player2.position, self.puck.position)
            max_dist = 250. / SCALE
            max_reward = -30.  # max (negative) reward through this proxy
            factor = max_reward / (max_dist * self.max_timesteps / 2)
            # Proxy reward for being close to puck in the own half
    ...
    def discrete_to_continous_action(self, discrete_action):
        ...
        action_cont = [(discrete_action == 1) * -1.0 + (discrete_action == 2) * 1.0,  # player x
                       (discrete_action == 3) * -1.0 +
                       (discrete_action == 4) * 1.0,  # player y
                       # player angle
        ...
        for obj in self.drawlist:
            for f in obj.fixtures:
                ...
                if type(f.shape) is circleShape:
                    pygame.draw.circle(self.surf, radius=f.shape.radius * SCALE,
                                       width=0, center=trans * f.shape.pos * SCALE, color=obj.color1)
                    pygame.draw.circle(self.surf, radius=f.shape.radius * SCALE,
                                       width=2, center=trans * f.shape.pos * SCALE, color=obj.color2)
                else:
    ...
    def mode(self, value: str | int | Mode):
        ...
        if isinstance(value, Mode):
            ...
            try:
                ...
                raise ValueError(
                    f"{value} is not a valid name for {Mode.__name__}")
            ...
            try:
                ...
                raise ValueError(
                    f"{value} is not a valid value for {Mode.__name__}")
        else:
            raise TypeError(
                "Input value must be an Enum, name (str), or value (int)")
...
class BasicOpponent():
    ...
    def act(self, obs, verbose=False):
        ...
        action = np.clip(error * [kp, kp / 5, kp / 2] -
                         v1 * need_break * [kd, kd, kd], -1, 1)
        if self.keep_mode:
